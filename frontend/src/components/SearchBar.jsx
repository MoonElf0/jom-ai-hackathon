...
// Fully theme-aware — all colours via CSS variables through sb-* classes.
...
const ClockIcon = () => (
  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor"
    strokeWidth="2" strokeLinecap="round" style={{ flexShrink: 0, opacity: 0.5 }}>
...
export default function SearchBar({ facilities = [], onSelectFacility }) {
  ...
  const hintTypes      = trimmed.length >= 2
  ...
  return (
    <div ref={wrapperRef} className="sb-wrap">
      ...
      <div className={`sb-input-row${showDropdown ? ' sb-open' : ''}${isFocused ? ' sb-focused' : ''}`}>
        ...
        />
        {query && (
          <button className="sb-clear-btn" onClick={() => setQuery('')} aria-label="Clear">
            <XIcon />
          </button>
        )}
      </div>
        ...
        <div className="sb-dropdown">
          ...
          <div className="sb-tabs">
            {['search', 'favourites'].map(tab => (
              <button
                key={tab}
                className={`sb-tab-btn${activeTab === tab ? ' active' : ''}`}
          ...
          {activeTab === 'search' && (
            <div>
              {/* Type hints */}
              {hintTypes.length > 0 && suggestions.length === 0 && (
              ...
              {/* No results */}
              {trimmed.length >= 2 && suggestions.length === 0 && hintTypes.length === 0 && (
              ...
              {trimmed.length === 0 && recentFacilities.length > 0 && (
                <div>
                  <div className="sb-recent-header">
                    <p className="sb-section-label" style={{ margin: 0 }}>Recent</p>
            ...
            <div>
              {favouriteFacilities.length > 0 ? (
                ...
                <div className="sb-fav-empty">
                  ...
                  <p className="sb-empty-sub">Tap the star on any facility to save it here</p>
                </div>
              )}
            </div>
          )}
